...
def exhibitor_already_scraped(exhibitor_name, base_dir="downloads"):
...
def ensure_china_filter_applied(page):
    """Check if China filter is applied by checking URL, if not, re-apply it"""
    try:
        # Check the current URL for f_country=CN parameter
        current_url = page.url
        china_filter_in_url = 'f_country=CN' in current_url

        if china_filter_in_url:
            print("  ✓ China filter still applied (f_country=CN in URL)")
            return True
        else:
            print("  ⚠️ China filter LOST (f_country=CN not in URL)! Re-applying...")

            # First check if "Show more" needs to be clicked
            show_more_buttons = page.locator('div.search-filter-options-show-all__link:has-text("Show more")').all()

            # Check if China filter is visible (if not, need to click "Show more")
            try:
                china_filter_visible = page.locator('span.search-filter-option__label:has-text("China")').first.is_visible(timeout=2000)
            except:
                china_filter_visible = False

            if not china_filter_visible and len(show_more_buttons) >= 2:
                print("  Expanding country list...")
                show_more_buttons[1].click()
                page.wait_for_timeout(1000)

            # Click China filter to activate it
            china_filter = page.locator('span.search-filter-option__label:has-text("China")').first
            china_filter.click()
            page.wait_for_timeout(5000)  # Increased wait time for filter to properly load

            # Verify it was applied by checking URL again
            new_url = page.url
            if 'f_country=CN' in new_url:
                print("  ✓ China filter applied successfully!")
                return True
            else:
                print("  ✗ Failed to apply China filter!")
                return False
    except Exception as e:
        print(f"  ⚠️ Error checking/applying China filter: {e}")
        return False
...
def run():
    ...
    with sync_playwright() as p:
        ...
        # Start with the base search page
        url = "https://www.medica-tradefair.com/vis/v1/en/search?_query=&f_type=profile"
        ...
        # Click the China checkbox filter FIRST
        print("\n" + "="*60)
        print("APPLYING CHINA FILTER")
        print("="*60 + "\n")

        try:
            # Wait for filters to be available
            page.wait_for_selector('span.search-filter-option__label', timeout=10000)

            # First, click the SECOND "Show more" button to reveal hidden country options
            print("Looking for Country/Region 'Show more' button (second one)...")
            show_more_buttons = page.locator('div.search-filter-options-show-all__link:has-text("Show more")').all()

            if len(show_more_buttons) >= 2:
                print(f"✓ Found {len(show_more_buttons)} 'Show more' buttons")
                print("✓ Clicking the SECOND 'Show more' button (Country/Region)...")
                show_more_buttons[1].click()  # Click the second one (index 1)
                page.wait_for_timeout(2000)  # Wait for expanded list
                print("✓ Expanded country list!")
            else:
                print(f"⚠️ Found only {len(show_more_buttons)} 'Show more' button(s), trying the first one...")
                if len(show_more_buttons) > 0:
                    show_more_buttons[0].click()
                    page.wait_for_timeout(2000)

            # Now find and click the China checkbox
            print("Looking for China filter...")
            china_filter = page.locator('span.search-filter-option__label:has-text("China")').first

            if china_filter.is_visible():
                print("✓ Found China filter, clicking...")
                china_filter.click()
                page.wait_for_timeout(5000)  # Wait for results to update
                print("✓ China filter applied successfully!")

                # Verify results updated - wait for exhibitor cards to reload
                page.wait_for_selector("article", timeout=10000)
                print("✓ Results updated!")
            else:
                print("✗ China checkbox not visible even after 'Show more'")
                print("ERROR: Cannot proceed without China filter!")
                browser.close()
                return
        except Exception as e:
            print(f"✗ Error applying China filter: {e}")
            print("ERROR: Cannot proceed without China filter!")
            browser.close()
            return
        ...
        try:
            page.wait_for_selector("div.teaser-tile-bounds", timeout=60000)
        except Exception as e:
        ...
        while True:
            ...
            print(f"Found {len(items)} exhibitors on this page.\n")

            for item_idx, item in enumerate(items):
                try:
                    # Check China filter BEFORE processing each exhibitor
                    if not ensure_china_filter_applied(page):
                        print("  ✗ Cannot ensure China filter - skipping this page")
                        break
                    ...
                    print(f"[{item_idx+1}/{len(items)}] {name} - {location}")
                    ...
                    print("  Clicking card to open modal...")
                    item.click()
                    page.wait_for_timeout(2000)
                    ...
                    # Check and re-apply China filter if needed (using URL check)
                    ensure_china_filter_applied(page)
            ...
            print(f"\n📊 Progress: {total_scraped} new | {total_skipped} skipped")
    ...
    print(f"Total SKIPPED (already downloaded): {total_skipped}")
    print(f"Data saved to: {base_download_dir}/")
    if all_exhibitors:
        print(f"New data JSON: exhibitors_chinese_new.json")
        print(f"New data CSV: {csv_file}")
    print(f"{'='*60}")
