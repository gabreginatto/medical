"""Update super_tenders view with R$15M minimum"""
import asyncio
...
async def update_view():
    ...
    try:
        ...
        print("Dropping old super_tenders view...")
        await conn.execute("DROP MATERIALIZED VIEW IF EXISTS analytics.super_tenders CASCADE")

        print("Creating new super_tenders view with R$15M minimum...\n")
...
"""
        ...
        print(f"📊 Found {count} super tenders (≥R$15M with 2-5 items)\n")
        ...
        print("Top 10 Super Tenders (≥R$15M):")
        print("="*110)
        for i, row in enumerate(results, 1):
            print(f"{i}. {row['organization_name'][:55]} ({row['state_code']})")
